...
export default async function handler(req, res) {
  ...
  if (!action || !ALLOWED_METHODS.includes(action)) {
    return res.status(400).json({ error: 'Invalid action. Allowed: ' + ALLOWED_METHODS.join(', ') });
  }
  ...
  if (allowedIds.length > 0 && !allowedIds.includes(String(chat_id))) {
    return res.status(403).json({ error: 'chat_id not allowed' });
  }
  ...
  try {
    ...
    if (action === 'sendMessage') {
      ...
      if (photo) {
        ...
        return res.status(400).json({ error: 'photo is required for sendPhoto' });
      }
    }
